...
class MicroworldsTwitterScraper(Scraper):
    ...
    # Twitter Scraper V2
    TWITTER_SCRAPER_ACTOR_ID = "61RPP7dywgiy0JPD0" 
    ...
    async def scrape(self, scrape_config: ScrapeConfig) -> List[DataEntity]:
        ...
        # random_second = random.randrange(24 * 60 * 60)
        # start_time = datetime.now() - one_day_delta - dt.timedelta(seconds=random_second)
        start_time = datetime.now() - one_day_delta
        end_time = start_time + dt.timedelta(hours=1)

        # start_time = scrape_config.date_range.start
        # end_time = scrape_config.date_range.start + dt.timedelta(seconds=1)

        query = f"#salvador since:{start_time.astimezone(tz=dt.timezone.utc).strftime(date_format)} until:{end_time.astimezone(tz=dt.timezone.utc).strftime(date_format)} -filter:replies"
        ...
        run_input = {
            **MicroworldsTwitterScraper.LATEST_RUN_INPUT,
            "searchTerms": [query],
            "maxItems": max_items,
        ...
        run_config = RunConfig(
            actor_id=MicroworldsTwitterScraper.TWITTER_SCRAPER_ACTOR_ID,
        ...
        # print(dataset[0])
        x_contents = self._best_effort_parse_dataset_2(dataset)
        # print(len(x_contents))
        bt.logging.success(
        ...
        l = 0
        for da in data_entities:
            l += da.content_size_bytes
        if data_entities:
            avg = l / len(data_entities)
            print(f"avg len: {avg}") 

        return data_entities

    def _best_effort_parse_dataset_2(self, dataset: List[dict]) -> List[XContent]:
        ...
        for data in dataset:
            try:
                ...
                results.append(
                    XContent(
                        username=utils.extract_user(data["twitterUrl"]),
...
async def test_validate():
    ...
    true_entities = [
        ...
        DataEntity(
            uri="https://twitter.com/DardaniaTimes/status/1762249130657689996",
            datetime=dt.datetime(2024, 2, 26, 22, 51, 48, tzinfo=dt.timezone.utc),
            source=DataSource.X,
            label=None,
            content=b'{"username": "@DardaniaTimes", "text": "Breaking news \\n\\n@KokalariEvi has destroyed foreign and domestic enemies of Kosova \\n\\nToday\\u2019s victim was @CekuFitim", "url": "https://twitter.com/DardaniaTimes/status/1762249130657689996", "timestamp": "2024-02-26T22:51:48+00:00", "tweet_hashtags": [], "model_config": {"extra": "ignore"}}',
            content_size_bytes=336,
        ),        
    ]
...
async def test_validate_2():
    scraper = MicroworldsTwitterScraper()

    true_entities = await scraper.scrape(
        ScrapeConfig(
            entity_limit=12,
            date_range=DateRange(
                start=dt.datetime(2024, 1, 30, 0, 0, 0, tzinfo=dt.timezone.utc),
